...
The aim of this analysis is to estimate the sample size for a study on the effects of sleep disruption and sleep duration on labor duration (the `pregnancy` study), with the goal of replicating the findings of @lee2004.

## Effect size estimation (Cohen's f)

See @cohen1988, @frey2022, and @maxwell2018 to learn more.

### Parameters

The parameters for the effect size were extracted from @lee2004, as shown below.

> Controlling for infant birth weight, ANCOVA indicated that the degree of prenatal sleep disruption \[WASO\] had a significant effect on labor duration (F[2,125] = 6.93, P = .001). [@lee2004, pp. 2044]

> Controlling for infant birth weight, ANCOVA indicated that prenatal sleep quantity (TST) also had a signiﬁcant eﬀect on labor duration (F[2,125] = 5.54, P = .005). [@lee2004, pp. 2044]
...
::: {#tbl-1 .g-col-10}
|            | n  | Labor duration* (mean ± SD) |
|------------|----|---------------------------------|
| **WASO**   |    |                                 |
| 15%+       | 41 | 26.0 ± 11.7†                    |
| 10%-14.9%  | 32 | 16.3 ± 12.9                     |
| <10%       | 58 | 18.3 ± 14.6                     |
| **TST**    |    |                                 |
| < 6 h      | 19 | 29.0 ± 12.5†                    |
| 6-6.9 h    | 38 | 20.5 ± 11.3                     |
| 7+ h       | 74 | 17.7 ± 15.6                     |

<p style="font-size: .9rem;color: #5a6570;">\*Adjusted for infant birth weight.</p>
<p style="margin-top: -0.25em; font-size: .9rem;color: #5a6570;">†Significantly longer labor than the other 2 groups.</p>

<p style="font-size: .9rem;color: #5a6570;text-align:center">Source: @lee2004 [pp. 2044].</p>
...
# Group 1: 15+
n_1_waso <- 41
mean_1_adj_waso <- 26 # Significantly longer labor than the other 2 groups.
sd_1_waso <- 11.7
...
n_total_waso <- sum(n_1_waso, n_2_waso, n_3_waso)
...
f_unb_waso <- 
  ((k - 1) / n_total_waso) %>%
  `*`(((n_total_waso - k - 2) / (n_total_waso - k)) * (f_test - 1)) |> 
  sqrt()
...
# Group 1: <6 h
n_1_tst <- 19
mean_1_adj_tst <- 29 # Significantly longer labor than the other 2 groups.
sd_1_tst <- 12.5
...
n_total_tst <- sum(n_1_tst, n_2_tst, n_3_tst)
...
f_unb_tst <- 
  ((k - 1) / n_total_tst) %>%
  `*`(((n_total_tst - k - 2) / (n_total_tst - k)) * (f_test - 1)) |> 
  sqrt()
...
#' @description
#'
#' `ancova_muhtpo()` compute the __power__ and  `ancova_muhtnp()` the
...
#'
#' See Cohen (1988) to learn more about power analysis .
#'
#' @details
#'
#' ## `contrast` parameter
